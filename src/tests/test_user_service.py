...
from src.package.user_service import UserService
...
@pytest.fixture
def user_builder():
    return UserBuilder().with_username("John Doe").with_email(
...
def test_register_user_missing_parameters(user_service, username, email, age, expected_error_message):
    with pytest.raises(ValueError) as excinfo:
        user_service.register_user(username, email, age)
    assert str(excinfo.value) == expected_error_message


def test_register_user(user_service, database_mock):
    # Arrange
...
def test_get_users(user_service, database_mock):
    # Arrange
    users = [
        UserBuilder().with_username("John Doe").with_email(
            "johndoe@example.com").with_age(30).build(),
        UserBuilder().with_username("Jane Doe").with_email(
            "janedoe@example.com").with_age(25).build()
    ]
...
def test_get_user_by_index(user_builder, user_service, database_mock):
    ...
    # Assert
    expected = {"username": user_builder.get_username(),
...
def test_get_user_by_index_not_found(user_service, database_mock):
    ...
    with pytest.raises(IndexError) as excinfo:
        user_service.get_user_by_index(1)

    # Assert
    database_mock.get.assert_called_once_with(1)
